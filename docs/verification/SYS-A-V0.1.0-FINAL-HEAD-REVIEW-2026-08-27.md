...
| Finding | Disposition |
|---|---|
| [Bind the release evidence to the reviewed commit](https://github.com/Neel-Error404/balcony-agent-bridge/pull/6#discussion_r3868675593) | **Resolved.** The final ladder and package freeze were repeated on the tested package source. The `v0.1.0` tag target must be the eventual final evidence-containing commit (the current tagged tree at release creation), so its GitHub source archive contains this authoritative evidence. The separately pinned npm package source remains `9458a50afeb28fb3f759de53f3889380af983e90`; it is not a claim about the eventual tag target. |
